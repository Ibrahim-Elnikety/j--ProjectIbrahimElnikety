...
class Type {
    ...
    /**
     * Maps j-- class to Java Classes
     * @return
     */
    public Class<?> toJavaClass() {
        if (this == Type.INT) return int.class;
        if (this == Type.BOOLEAN) return boolean.class;
        if (this == Type.CHAR) return char.class;
        if (this == Type.LONG) return long.class;
        if (this == Type.FLOAT) return float.class;
        if (this == Type.DOUBLE) return double.class;
        if (this == Type.STRING) return String.class;
        if (this == Type.VOID) return void.class;

        // For reference types, use the JVM name
        try {
            return Class.forName(this.jvmName().replace('/', '.'));
        } catch (ClassNotFoundException e) {
            return Object.class; // safe fallback
        }
    }
    ...
    public static String descriptorFor(Class<?> classRep) {
...
class TypeName extends Type {
    ...
    public Type resolve(Context context) {
        ...
        if (resolvedType == null) {
            try {
                // First try the given name directly
                resolvedType = typeFor(Class.forName(name));
                context.addType(line, resolvedType);
            } catch (Exception e1) {
                // Fallback: implicit java.lang import for unqualified names
                try {
                    String jlName = "java.lang." + name;
                    resolvedType = typeFor(Class.forName(jlName));
                    context.addType(line, resolvedType);
                } catch (Exception e2) {
                    JAST.compilationUnit.reportSemanticError(line,
                            "Unable to locate %s", name);
