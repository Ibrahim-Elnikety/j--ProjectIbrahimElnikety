...
class JPostIncrementOp extends JUnaryExpression {
    ...
    public JExpression analyze(Context context) {
        if (!(operand instanceof JLhs)) {
            JAST.compilationUnit.reportSemanticError(line, 
                "Operand to ++ must have an LValue.");
            type = Type.ANY;
        } else {
            operand = (JExpression) operand.analyze(context);
            operand.type().mustMatchExpected(line(), Type.INT);
    ...
    public void codegen(CLEmitter output) {
        if (operand instanceof JVariable) {
            // Local variable; otherwise analyze() would replace with field selection.
            int offset = ((LocalVariableDefn) ((JVariable) operand).iDefn()).offset();
            if (!isStatementExpression) {
                // For value context, push original rvalue first (post-increment semantics).
                operand.codegen(output);
            }
            output.addIINCInstruction(offset, 1);
        } else {
            // General LHS (field/array element)
            ((JLhs) operand).codegenLoadLhsLvalue(output);   // address
            ((JLhs) operand).codegenLoadLhsRvalue(output);   // original value
            if (!isStatementExpression) {
                // Duplicate original rvalue for expression result
                ((JLhs) operand).codegenDuplicateRvalue(output);
            }
            output.addNoArgInstruction(ICONST_1);
            output.addNoArgInstruction(IADD);
            ((JLhs) operand).codegenStore(output);           // write back incremented value
        }
    }
}
