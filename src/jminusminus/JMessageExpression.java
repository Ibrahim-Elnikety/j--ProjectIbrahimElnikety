...
class JMessageExpression extends JExpression {
	...
	public JExpression analyze(Context context) {
		...
		// Then analyze the arguments, collecting their types (in Class form) as
		// argTypes.
		...
		if (method == null) {
			try {
				Class<?> clazz = target.type().toJavaClass();
				Class<?>[] paramClasses = new Class<?>[argTypes.length];
				for (int i = 0; i < argTypes.length; i++) {
					paramClasses[i] = argTypes[i].toJavaClass();
				}
				java.lang.reflect.Method m = clazz.getMethod(messageName, paramClasses);
				method = new Method(m);
				type = Type.typeFor(m.getReturnType());

				// Apply access checks uniformly
				context.definingType().checkAccess(line(), (Member) method);

				// Non-static method cannot be referenced from a static context.
				if (!method.isStatic()) {
					if (target instanceof JVariable && ((JVariable) target).iDefn() instanceof TypeNameDefn) {
						JAST.compilationUnit.reportSemanticError(line(),
			...
			if (!method.isStatic()) {
				if (target instanceof JVariable && ((JVariable) target).iDefn() instanceof TypeNameDefn) {
					JAST.compilationUnit.reportSemanticError(line(), "Non-static method "
							+ Type.signatureFor(messageName, argTypes) + " cannot be referenced from a static context");
				}
			}
		}
		return this;
	...
	public void codegen(CLEmitter output) {
	    if (method == null) {
	        JAST.compilationUnit.reportSemanticError(line(),
	            "Codegen invoked with unresolved method: " + messageName +
